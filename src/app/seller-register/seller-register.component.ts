...
export class SellerRegisterComponent implements OnInit {
 ...
 this.SellerRegisForm= new FormGroup({
  ...
  LastName:new FormControl ('', [Validators.required , Validators.pattern('[A-Za-z]{3,}')]),
   CompanyName: new FormControl ('', [Validators.required , Validators.pattern('[A-Za-z]{3,}')]),
      Email: new FormControl ('',[Validators.required , Validators.pattern('[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}')]),
      Password:  new FormControl ('',[Validators.required,Validators.pattern('[a-z0-9._%+-]{6,}')]),
      Phone: new FormControl ('',Validators.pattern('[0-9]{7,}')),
      Address:new FormGroup({
        City: new FormControl ('',Validators.pattern('[A-Za-z]{3,}')),
        Street: new FormControl ('',[Validators.required]),
        BulNo:new FormControl ([Validators.required,Validators.pattern('[0-9]{1,}')]),
      }) ,
      Payment:new FormGroup({
        CreditCard:new FormControl('',Validators.pattern('[1-9]{9,}')),
        CVV:new FormControl ('',[Validators.required]),
        CardHolderName:new FormControl ('',[Validators.required,Validators.pattern('[A-Za-z]{3,}')]),
      }) ,
      Order:new FormArray([
       new FormGroup ({
      ...
      Products:new FormArray([
        new FormGroup ({
          Product_Id:new FormControl (doc(this.firestore,'Products','xuw7NPQgRP2twaCSX7Sd')),
       })])
  ...
  Signup() {
    ...
    this.AuthService.SignupSeller(SellerModel.Email!,SellerModel.Password!).subscribe(() => {
      if (this.AuthService.userID) {
        this.errorMessage = '';
        this.SellerServ.registerSeller(this.AuthService.userID, SellerModel).then(() => {
